...
# Create a dictionary to hold I_j values and their differences from 0.065
I_j_values = {}
...
for j in range(26):
    I_j = 0
    for i in range(26):
        char_from_english = chr(i + 97)  # Lowercase letter from English frequency
        char_from_ciphertext = chr((i + j) % 26 + 97)  # Shifted lowercase letter for ciphertext frequency
        if char_from_ciphertext in ciphertext_freq:  # Ensure the shifted char exists in ciphertext frequency
            I_j += english_freq[char_from_english] * ciphertext_freq[char_from_ciphertext]
    I_j_values[j] = I_j
    diff_from_0_065 = abs(I_j - 0.065)  # Calculate the difference from 0.065
    print(f"I_{j} - 0.065 = {diff_from_0_065:.5f}")
